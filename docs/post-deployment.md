...
| Desktop appears, but connecting fails with *"the credentials did not work"* | The tenant has not opted in to Entra ID authentication for RDP, or the user is missing **Virtual Machine User Login** on `rg-<env>-avd`, or the host pool lost `enablerdsaadauth:i:1`. | Run `./scripts/enable-rdp-sso.ps1`; grant the role; confirm the RDP property with `azd provision`. |
